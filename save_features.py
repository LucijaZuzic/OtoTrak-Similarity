...
for vehicle1 in all_possible_trajs[window_size].keys():  
    for r1 in all_possible_trajs[window_size][vehicle1]:
        for x1 in all_possible_trajs[window_size][vehicle1][r1]: 
            ...
            #print(all_feats_trajs[window_size][vehicle1][r1][x1])
            for feat_name in all_feats_trajs[window_size][vehicle1][r1][x1]: 
                if "poly" in feat_name: 
                    for val in list(all_feats_trajs[window_size][vehicle1][r1][x1][feat_name]): 
                        new_csv_content += str(val) + ","
                    if len(list(all_feats_trajs[window_size][vehicle1][r1][x1][feat_name])) == 0:
                        for d in range(deg + 1): 
                            new_csv_content += ","
                else:
                    new_csv_content += str(all_feats_trajs[window_size][vehicle1][r1][x1][feat_name]) + ","
